...
export default function RentCalculator() {
  ...
  const calculateRent = () => {
    if (selectedCategory && numberOfDays > 0) { 
      ...
      if (selectedCategoryData) {
        ...
        const total = rentAmount * numberOfDays;
        setTotalRent(total);
      }
    }
  };
  ...
  <div className="bg-white p-8 rounded-lg shadow-lg h-3/4 w-4/5 mt-20">
    <h2 className="text-3xl font-semibold mb-6">Rent Calculator</h2>
    <div className="mb-6">
      <label className="block text-gray-600">Select Equipment:</label>
      <select
    ...
    <button
      ...
      className="bg-blue-500 text-white font-semibold py-3 px-6 rounded-md hover:bg-blue-600 focus:outline-none"
    >
      Calculate
    </button>
    {totalRent > 0 && (
      <p className="mt-6">
        Total Rent: <strong className="text-green-500 text-xl">${totalRent}</strong>
      </p>
    )}
  </div>
</div>
